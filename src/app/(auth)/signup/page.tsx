...
import { Mail, Lock, User, UtensilsCrossed, ArrowRight } from "lucide-react";
  ...
  return (
    <main className="flex min-h-screen items-center justify-center overflow-hidden bg-paper-dim px-4 py-12">
      <div
        ...
        style={{ background: "radial-gradient(circle at 80% 10%, var(--color-saffron) 0%, transparent 45%)" }}
      />
      <motion.div
      ...
      >
        ...
        <p className="mt-1.5 text-center text-sm text-ink-soft">Start hosting in under two minutes</p>

        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          <Input label="Full name" icon={<User className="h-4 w-4" />} placeholder="Aditi Rao" required />
          <Input label="Email" type="email" icon={<Mail className="h-4 w-4" />} placeholder="you@example.com" required />
          <Input label="Password" type="password" icon={<Lock className="h-4 w-4" />} placeholder="Create a password" required />
        ...
        <div className="space-y-2.5">
          <Button
            ...
            className="w-full"
            onClick={() => {
              showToast({ title: "Signed up with Google", kind: "success" });
              router.push(ROUTES.dashboard);
            }}
          >
        ...
        <p className="mt-8 text-center text-sm text-ink-soft">
          ...
          <Link href={ROUTES.login} className="font-semibold text-ember hover:underline">
            Log in
          </Link>
        </p>
      </motion.div>
