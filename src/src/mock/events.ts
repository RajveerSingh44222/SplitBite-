...
export function buildSeedEvent(): PartyEvent {
  ...
  const participants: Participant[] = [
    ...
    },
    {
      ...
      cartValue: 0,
      isHost: false,
      joinedAt: new Date(now - 5 * 60 * 1000).toISOString(),
    },
  ];
...
export function buildActivityFeed(eventId: string): ActivityItem[] {
  ...
  const items: Omit<ActivityItem, "id" | "eventId">[] = [
    ...
    { type: "joined", actorName: "Aditi Rao", actorAvatar: currentUser.avatar, message: "you joined the event", timestamp: new Date(now - 5 * 60000).toISOString() },
...
export function buildNotifications(): Notification[] {
  ...
  return [
    ...
    { id: randomId("ntf"), title: "Welcome to PartyPlatter", body: "Create your first event and invite your crew", timestamp: new Date(now - 96 * 3600 * 1000).toISOString(), read: true, kind: "system" },
  ];
}
